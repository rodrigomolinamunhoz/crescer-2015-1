...
public class Netflox {
	...
	public void reproduzirFilme(Filme filme) {
		if (ListaAcervoFilmes.contains(filme)) {
			if (ListaReproduzidos.containsKey(filme.getGenero())) {
				...
				ListaReproduzidos.put(filme.getGenero(),
						new ArrayList<>(Arrays.asList(filme)));
			}
		}
	}
	...
	public String gerarRelatorio() {
		...
		for (Map.Entry<Genero, ArrayList<Filme>> parChaveValor : ListaReproduzidos
				.entrySet()) {
			System.out.println(parChaveValor.getKey() + " - "
					+ parChaveValor.getValue().size());
			relatorio.append(parChaveValor.getKey() + " - "
					+ parChaveValor.getValue().size());
		}
